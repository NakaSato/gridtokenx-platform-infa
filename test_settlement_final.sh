...
# Register Buyer 7/Seller 7
echo "--- Registering Users (Funding Triggered) ---"
B_TOKEN=$(curl -s -X POST http://127.0.0.1:4000/api/v1/users \
  -H "Content-Type: application/json" \
  -d '{"username": "buyer7", "email": "buyer7@test.com", "password": "password123", "first_name": "B", "last_name": "7"}' | jq -r '.auth.access_token')

S_TOKEN=$(curl -s -X POST http://127.0.0.1:4000/api/v1/users \
  -H "Content-Type: application/json" \
  -d '{"username": "seller7", "email": "seller7@test.com", "password": "password123", "first_name": "S", "last_name": "7"}' | jq -r '.auth.access_token')
